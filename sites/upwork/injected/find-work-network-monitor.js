(() => {
    if (window.__proposalCopycatFindWorkNetworkMonitorInstalled) {
        return;
    }

    const SOURCE = 'proposal-copycat-find-work-monitor';
    const EVENT_TYPE = 'graphql-response';
    const TARGET_ALIAS = 'bestMatchRecommendationsFeed.retrieve';
    const TARGET_ORIGIN = 'https://www.upwork.com';
    const GRAPHQL_PATH_HINTS = ['/api/graphql/'];
    const REQUEST_OPERATION_HINTS = [
        TARGET_ALIAS.toLowerCase(),
    ...
    const LOG_PREFIX = '[ProposalCopycatFindWorkMonitor]';
    ...
    const safeParseJsonPayload = (rawText) => {
    ...
    const isLikelyFindWorkJobResult = (item) => {
        ...
        return !!(
            ...
            (
                /^~0\d+/.test(ciphertext) ||
                /\/jobs\//i.test(String(item.url || item.jobPostUrl || item.job?.url || ''))
            ) &&
            description.length > 0
        );
    };

    const hasFindWorkPayloadSignal = (responseText) => {
        const parsed = safeParseJsonPayload(responseText);
        if (!parsed) {
            return false;
        }

        const directResults = parsed?.data?.bestMatchRecommendationsFeed?.results;
        if (
            Array.isArray(directResults) &&
            directResults.some((entry) => isLikelyFindWorkJobResult(entry))
        ) {
            return true;
        }

        const objectNodes = collectObjectNodes(parsed);
        for (const node of objectNodes) {
            if (!node || typeof node !== 'object' || Array.isArray(node)) {
                continue;
            }

            const results = Array.isArray(node.results) ? node.results : null;
            if (!results || results.length === 0) {
                continue;
            }

            if (results.some((entry) => isLikelyFindWorkJobResult(entry))) {
                return true;
            }
        }
    ...
    const detectMatchReason = ({ parsedUrl, requestBodyText, responseText }) => {
        ...
        if (hasFindWorkPayloadSignal(responseText)) {
    ...
    const buildPayload = ({
        ...
        const requestBodyDecoded = decodeBodyForMatching(requestBodyText);
        ...
        return {
            ...
            transport,
            path: shortPathFromUrl(requestUrl),
            url: requestUrl,
            method: String(requestMethod || 'GET').toUpperCase(),
            status,
            ok,
            graphqlAlias: String(parsedUrl?.searchParams?.get('alias') || ''),
            pageUrl: window.location.href,
            pageTitle: document.title || '',
            requestStartedAtMs: Number(requestStartedAtMs) || Date.now(),
            capturedAtMs: Date.now(),
            responseTextLength: responseText.length,
            requestBodySnippet: String(requestBodyDecoded || requestBodyText || '').slice(0, 500),
    ...
    if (typeof window.fetch === 'function') {
        ...
        window.fetch = async function proposalCopycatPatchedFetch(input, init) {
            ...
            const payload = buildPayload({
                parsedUrl,
                requestUrl: normalizedUrl || requestUrl,
                requestMethod,
                requestStartedAtMs,
                requestBodyText,
                responseText,
    ...
    if (window.XMLHttpRequest && XMLHttpRequest.prototype) {
        ...
        XMLHttpRequest.prototype.open = function proposalCopycatPatchedOpen(method, url) {
            this.__proposalCopycatFindWorkRequestMeta = {
        ...
        XMLHttpRequest.prototype.send = function proposalCopycatPatchedSend(body) {
            const meta = this.__proposalCopycatFindWorkRequestMeta || {};
            const requestStartedAtMs = Date.now();
    ...
    window.__proposalCopycatFindWorkNetworkMonitorInstalled = true;
    // eslint-disable-next-line no-console
