...
function renderSiteTabs(activeSiteId) {
    ...
    for (const site of SITE_REGISTRY) {
        ...
        btn.innerHTML = `<span class="tab-icon">${site.icon}</span> ${site.label}`;
...
async function activateSite(siteId) {
    ...
    try {
        ...
        // Execute panel JS
        const scriptUrl = chrome.runtime.getURL(`sites/${normalizedId}/panel.js`);
        ...
        contentEl.innerHTML = `<div class="coming-soon">
